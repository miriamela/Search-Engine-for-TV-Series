...
function searchSeries(ev) {
  series = [];
  ev.preventDefault();
  fetch(`https://api.tvmaze.com/search/shows?q=${title.value}`)
...
function renderingSeries() {
  ...
  for (let i = 0; i < series.length; i++) {
    let favourtiesindex = favourites.findIndex(
      (EachfavID) => EachfavID.showID === series[i].showID
    );
...
function addFav(event) {
  ...
  let selectedSeries = series.find(
    (eachSeries) => eachSeries.showID === parseInt(resultsId)
  ...
  let selectedSeriesIndex = favourites.findIndex(
    (eachSeries) => eachSeries.showID === parseInt(resultsId)
  );
...
function deleteFav(event) {
  ...
  let eliminatedSeriesIndex = favourites.findIndex(
    (eachFav) => eachFav.showID === parseInt(chosenEliminateId)
  );
